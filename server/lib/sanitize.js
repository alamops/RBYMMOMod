...
// the client's own Experience formula. Eight rather than BATTLE_MON_MAX because
// a co-op faint is split across two parties, and one number that covers both
// shapes beats two that have to be kept in step -- it bounds a foreign value
// before it enters a formula, it does not restate a game rule.
const PARTICIPANTS_MAX = 8;
 ...
 *              many shares split it. Not an amount: the hub holds no species
 *              table and can never compute one, so the referee states the
 *              facts and each client runs its own Experience formula over its
 *              own party.
 *
