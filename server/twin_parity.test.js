...
const {
  PLAYER_ID_HEX, BATTLE_EVENT_TYPES, MOVE_NAME_MAX, cleanBattleMove,
...
test('MOVE_NAME_MAX matches Config / Wire / sanitize', () => {
  // PROTOCOL 23's `name` is prose, and a limit that differs by one character is
...
test('a battle move sheet accepts a display name and survives without one', () => {
  ...
  assert.strictEqual(cleanBattleMove(base).name, undefined,
    'a protocol-22 sheet states none, and that is not a refusal');
