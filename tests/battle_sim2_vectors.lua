-- src/BattleSim2 against the Gen2 vector pack.
--
-- Run: luajit tests/battle_sim2_vectors.lua      (from this folder's root)
--   or luajit mods/rby_mmo/tests/battle_sim2_vectors.lua  (from the engine)
--
...
-- but the whole claim src/BattleSim2 makes is that it is pure arithmetic with
...
--
-- tests/fixtures/battle_sim2_vectors.json is the contract, not this file: the
-- future Node twin under server/lib/battle2/ will run the identical vectors, and a case
...
local function need(name)
  ...
  if not body then
    io.stderr:write("battle_sim2_vectors: missing " .. path .. "\n")
    os.exit(1)
  end
  local chunk, err = loadstr(body, "@" .. name .. ".lua")
  if not chunk then
    io.stderr:write("battle_sim2_vectors: " .. tostring(err) .. "\n")
    os.exit(1)
  end
...
local BattleSim = need("BattleSim2/init")
...
local FIXTURE = ROOT .. "/tests/fixtures/battle_sim2_vectors.json"
local raw = slurp(FIXTURE)
if not raw then
  io.stderr:write("battle_sim2_vectors: missing " .. FIXTURE .. "\n")
  os.exit(1)
end
...
each("damage", function(input)
  return Damage.compute(
    ...
    { power = input.power },
    {
      ...
      itemBoostPercent = input.itemBoostPercent,
    })
end)
...
each("accuracy", function(input)
  local hit, effective = Accuracy.hit(input.accuracy, input.roll, {
    accuracyStage = input.accuracyStage,
    evasionStage = input.evasionStage,
...
each("crit", function(input)
  local isCrit, chance, level = Crit.check(input.criticalLevel, input.roll, {
    focusEnergy = input.focusEnergy,
    highCritMove = input.highCritMove,
    scopeLens = input.scopeLens,
    speciesItemBonus = input.speciesItemBonus,
  })
  return { isCrit = isCrit, chance = chance, level = level }
end)
...
io.write(string.format("battle_sim2_vectors: %d passed, %d failed (%s)\n",
