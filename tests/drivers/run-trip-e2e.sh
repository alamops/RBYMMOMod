#!/usr/bin/env bash
#
# Three-client PvE e2e: one party of three on the Node hub, coop_npc then
# coop_wild. All JOIN; no in-game host.
#
#   bash mods/rby_mmo/tests/drivers/run-trip-e2e.sh
...
MAX="${MMO_HUB_MAX:-6}"
RUN_ID="${MMO_RUN_ID:-$$}"
HUB_LOG="/tmp/rby_mmo_trip_server_$$.log"
SHOT_DIR="${SHOT_DIR:-/tmp/rby_mmo_trip_shots-$RUN_ID}"
SYNC_DIR="${MMO_SYNC_DIR:-/tmp/rby_mmo_trip_sync-$RUN_ID}"
...
HUB_HOME="$(mktemp -d "${TMPDIR:-/tmp}/rby_mmo_trip_cfg.XXXXXX")"
...
hub_cli init --yes --host 127.0.0.1 --port "$PORT" --max "$MAX" \
  --log-level info >"$HUB_LOG" 2>&1 || exit 1
...
for role in "${ROLES[@]}"; do
  id_var="ID_${role}"
  log="/tmp/rby_mmo_trip_${role}_$$.log"
  ...
  if command -v stdbuf >/dev/null 2>&1; then
    MMO_ROLE="$role" MMO_HUB_CODE="$JOIN_CODE" SHOT_DIR="$SHOT_DIR" \
      ...
      POKEPORT_DRIVER="$DRIVERS/mmo_trip.lua" \
      stdbuf -oL -eL love . >"$log" 2>&1 &
  else
    MMO_ROLE="$role" MMO_HUB_CODE="$JOIN_CODE" SHOT_DIR="$SHOT_DIR" \
      MMO_SYNC_DIR="$SYNC_DIR" POKEPORT_IDENTITY="${!id_var}" \
      POKEPORT_DRIVER="$DRIVERS/mmo_trip.lua" love . >"$log" 2>&1 &
  fi
...
for role in "${ROLES[@]}"; do
  ...
  echo "  ---- guest $role ----"
  grep -E "MMO_TRIP_${upper}:|TIMEOUT" "${!log_var}" | sed 's/^/  /'
...
echo "  RESULT: three-client party PvE passed. Screenshots in $SHOT_DIR"
exit 0
